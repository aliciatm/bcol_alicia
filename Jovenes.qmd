...
top_propiedad_2011_filtro <- compra11 %>%
  arrange(desc(Compra11Prov)) %>%
  distinct(CPRO, .keep_all = TRUE) |> 
  select(Compra11Prov, MUN_LITERAL, PROV_LITERAL)
...
-   2011 con filtro. Se posiciona en primer lugar Cádiz (76%), seguido de Alicante, precisamente provincias con alto turismo.
...
-   Propiedad de 2021 - Compra de 2011. Doonde mas ha aumentado son muchos municipios rurales... me precupa no medir bien la compra solo en propiedad en vez de herencia.
...
compra_total %>%
  ...
  arrange(desc(OtroDifProv)) %>%
    select(OtroDifProv, MUN_LITERAL, PROV_LITERAL, CCAA)
