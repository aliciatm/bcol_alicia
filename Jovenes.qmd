...
# Jóvenes con filtro y propiedad
...
propiedad |> 
  filter(cusec == 23991) %>%
  select(PropiedadDifMun, AlquilerDifMun, OtroDifMun) %>%
  table()
...
cusec_count <- compra21 %>%
  group_by(cusec) %>%
  summarise(frequency = n())

# Unir las columnas MUN_LITERAL y PROV_LITERAL sin agrupar por ellas
cusec_count_full <- compra21 %>%
  select(cusec, MUN_LITERAL, PROV_LITERAL) %>%
  distinct() %>%
  inner_join(cusec_count, by = "cusec")

prop.table(p) * 100
```

##ver lo de herencia y otro
