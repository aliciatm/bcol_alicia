...
Joven de 18 a 35 años que no viva con los padres (ni otro fammiliar), viva en tenencia de 'compra'

-   Buscar variables (2011 y 2021)
-   Comparar jóvenes que compran según encuesta CIS 2010

## En 2011

Puede ser hipoteca heredada.
...
compra11 <- compra11 |> 
  filter(EDAD %in% c(18:35), # Jóvenes
         TENEN %in% c(1, 2), # Que viven en una casa comprada
...
compra21 <- microdatos2021 |> 
  select(CPRO, CMUN, VAREDAD, ANAC, TENEN_VIV, NORDEN, RESI_NACIM, VARANORES, VARANOM, CMUN_ANT, CMUN_UNANO, RESI_UNANO, CMUN_DANO, RESI_DANO, LTRAB, CMUN_TRAB, LEST, CMUN_EST, NORDEN_PAD, NORDEN_MAD, NORDEN_CON, NORDEN_OPA, TIPOPER, SEXO_MAD, SEXO_PAD, ESTRUC_HOG) 

compra21 <- compra21 |> 
  filter(VAREDAD %in% c(18:35), # Jóvenes
         TENEN_VIV == 2, # Que viven en propiedad
         TIPOPER != "H", # Que no son hijos en el núcleo
...
Hay menos jóvenes con las condiciones especificadas en 2021 que en 2011. La media de edad en 2011 es de 31,1, en 2021 es de 30,35.
...
table(compra21$NORDEN)

(prop.table(table(compra11$NORDEN)) * 100)
...
compra11 <- compra11 |> 
  group_by(cusec) |>  
  mutate(PoblacionMunicipios = n()) |> 
  ungroup() |> 
  mutate(PorcentajeMunicipios = (PoblacionMunicipios / n()) * 100)

compra11 |> 
    distinct(cusec, .keep_all = TRUE) |> 
  arrange(desc(PorcentajeMunicipios)) |> 
    select(PorcentajeMunicipios, MUN_LITERAL, PROV_LITERAL)
...
compra11 <- compra11 |> 
  group_by(CPRO) |>  
  mutate(PoblacionProvincias = n()) |> 
  ungroup() |> 
  mutate(PorcentajeProvincias = (PoblacionProvincias / n()) * 100)
...
mapa <- compra11 |> 
  select(cusec, PoblacionMunicipios, PorcentajeMunicipios)
  
mapa <- mapa |> 
  left_join(munic, by = "cusec")
  
ggplot(esp_can) +
    geom_sf() +
    geom_sf(data = can_prov) +
    geom_sf(data = can_box) + 
    geom_sf(data = mapa, aes(fill = PoblacionMunicipios), color = "black", size = 1) + 
    theme_void() +  
    labs(title = "Probabilidad de compra por municipio") +  
    theme(legend.position = c(0.18, 0.55),    
          legend.justification = "center",   
