...
pop <- microdatos2021_rec |> 
  group_by(CPRO) |> 
  summarize(PorcentajeRural = round(mean(HABITAT == "Rural") * 100, 2)) |> 
  print(arrange(desc(PorcentajeRural))) |> 
  ungroup()

#pop$TENEN_VIV <- as.numeric(pop$TENEN_VIV)

#pop <- pop |> 
#  left_join(viviendas21, by = "CPRO")

#  cor.test(pop$PorcentajeRural, pop$TENEN_VIV, method = "pearson")
...
modelo <- lm(PRINCIPAL == "Vacía" ~ HABITAT, data = viviendas11)
