...
A nivel de municipio, mayores de 10000 habitantes. Pierdo info de municipios menos de 10k. Imputarlos?
...
ipva <- ipva |> 
  left_join(cusec_rural_2011, by = c("cusec" = "cusec_original")) |> 
  mutate(cusec = coalesce(cusec.y, cusec)) |> 
  select(-cusec.y) |> 
  
  group_by(cusec) |> # Group all small mun together by province 
  summarise(ipva2021 = mean(ipva2021),
            ipva2011 = mean(ipva2011)) |> 
  ungroup()
...
ipv <- read_xlsx("Índice precios vivienda/ipv.xlsx") |> 
  ...
  rename(
    ...
    ipv2011Segunda = `ipv2011Vivienda segunda mano`) |>             mutate(CCAA = case_when(
...
nacionalidad <- nacionalidad |> 
  ...
  select(-cusec.y) |> 
  group_by(cusec) |> # Group all small mun together by province 
  summarise(PorcentajeEspañola = round(mean(PorcentajeEspañola), 2),
            PorcentajeEuropea = round(mean(PorcentajeEuropea), 2),
            PorcentajeNoEuropea = round(mean(PorcentajeNoEuropea), 2)) |> 
  ungroup() 
...
hipotecas <- hipotecas |> 
  mutate(DifPrecioHipotecaProv = ((PrecioHipoteca2021Prov - PrecioHipoteca2011Prov) / PrecioHipoteca2011Prov) * 100) |>     mutate(DifPrecioHipotecaProv = round(DifPrecioHipotecaProv, 2))
...
municipios <- municipios |>
  ...
  select(-ipva2021, -ipva2011) |> 
  filter(rowSums(is.na(select(., everything()))) > 0)
...
municipiosr <- municipios |> 
  select(-ipva2011, -ipva2021)

data_imputed <- data.frame(
  cusec = municipiosr$cusec,
  original = municipiosr$PrecioAlquilerM2,
  imputed_median = replace(municipiosr$PrecioAlquilerM2, is.na(municipiosr$PrecioAlquilerM2), median(municipiosr$PrecioAlquilerM2, na.rm = TRUE)),
  imputed_normboot = complete(mice(municipiosr, m=10, method = "norm.boot", seed=123))$PrecioAlquilerM2,
  imputed_pmm = complete(mice(municipiosr, m=10, method = "pmm", seed=123))$PrecioAlquilerM2,
  imputed_rf = complete(mice(municipiosr, m=10, method = "rf", seed=123))$PrecioAlquilerM2
) 

View(data_imputed|> 
  filter(rowSums(is.na(data_imputed)) > 0))
...
variables_independientes <- municipios[, c(9:47)]  # Seleccionar desde la novena columna hasta la cuadragésima columna
...
-   Navarra, falta de datos de alquiler por metro en alquiler.
